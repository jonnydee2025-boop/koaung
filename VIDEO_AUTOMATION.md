...
When a render starts (admin **Render Next**, Telegram `/render_next`, or a **scheduled** /
**repeat** due-time poll), the bot reserves **one** row and sets `status=processing`.
Batch member rows (non-anchor rows in a multi-row **Select Rows** rule) are never picked
directly — only the anchor row starts a batch job:
...
| **Render Next** / `/render_next` | **`do` rows only** |
...
**`pending`** rows are never picked automatically. Set a row to **`do`** and use
**Render Next**, or **Schedule** it when you want it rendered on a timer.
...
5. YouTube **title** comes from `dhamma_title` (batch: joined with ` | `). **Description** and **tags** are generated by Gemini when at least one API key is configured; otherwise the sheet `description` is used and no tags are sent.
...
When at least one Gemini API key is configured, the bot calls **Google Gemini** before each YouTube upload with the row's monk name and `dhamma_title`. Gemini returns JSON metadata defined by your prompt settings (default fields below):
...
If Gemini is unavailable or generation fails, the upload falls back to the sheet `description` column and sends no tags. Upload retries reuse the same generated description/tags without calling Gemini again.

### API keys and model failover

Configure keys and models in **Settings → AI** (admin panel):

- **API keys (failover order)** — Key 1 is tried first. If Google returns quota or rate-limit errors, the bot automatically retries with key 2, then key 3, and so on. Keys are stored in `gemini_api_keys.json` on the server (gitignored).
- **Model fallback** — Within each key, the primary model is tried first, then fallback models if the call fails for non-quota reasons.

Bootstrap: if `gemini_api_keys.json` is empty but `GEMINI_API_KEY` is set in `.env`, that env key is used as slot 1 until you save keys from the admin UI.

| Variable | Purpose |
|----------|---------|
| `GEMINI_API_KEY` | Optional bootstrap Gemini key (used when no keys file) |
| `GEMINI_API_KEYS_PATH` | Path to ordered keys JSON (default: `gemini_api_keys.json`) |
| `GEMINI_MODEL` | Model id (default: `gemini-2.0-flash`) |
| `GEMINI_MODEL_FALLBACKS` | Comma/space-separated fallback models |
| `GEMINI_SETTINGS_PATH` | Path to model settings JSON (default: `gemini_models.json`) |
...
| `GEMINI_API_KEY` | Optional bootstrap Gemini key; manage ordered failover keys in Settings → AI |
| `GEMINI_API_KEYS_PATH` | Path to ordered keys JSON (default: `gemini_api_keys.json`) |
...
| `video_bot/scheduler.py` | Scheduled / repeat due-time poll loop |
...
- Do not commit `.env`, `token.json`, `client_secret.json`, `row_range_rules.json`, `repeat_jobs.json`, `gemini_models.json`, `gemini_api_keys.json`, or `gemini_youtube_prompt.json`.
